...
// Real decision rationale generated by Claude (the project's AI reasoning layer)
// for this exact Sarno critical state — shown to illustrate the agent's
// natural-language explanation. The live version is produced keylessly by
// scripts/ai-brief.sh against the local Claude Code CLI.
const RATIONALE =
  "I am authorizing a 500 USDC relief payout to registered Sarno wallets because the region has crossed every parametric trigger encoded on-chain: 89% model risk against a 70% threshold, ~146mm of cumulative rainfall over 72 hours, 34mm/h peak intensity, and 0.82 terrain susceptibility on already-saturated soil. I cross-checked this signal against public reporting on conditions in Sarno, Campania, and it corroborates the on-chain readings, giving me independent confidence that the event is real and not a sensor or data artifact. The payout is bounded entirely by hard on-chain rules — fixed amount, threshold-gated, and restricted to pre-registered wallets — so I cannot exceed the mandated disbursement or pay anyone outside the verified registry.";
...
export default function SimPanel() {
  ...
  function typeRationale() {
    if (typer.current) clearInterval(typer.current);
    let n = 0;
    typer.current = setInterval(() => {
      n += 2;
      setAi(RATIONALE.slice(0, n));
      if (n >= RATIONALE.length && typer.current) {
    ...
    <div className="sim">
      ...
      {ai && (
        <div className="sim-ai">
          <div className="sim-ai-h">
            <span className="sim-ai-badge">AI rationale · Claude</span>
            <span className="sim-ai-note">natural-language decision explanation</span>
          </div>
          <p className="sim-ai-body">
            {ai}
